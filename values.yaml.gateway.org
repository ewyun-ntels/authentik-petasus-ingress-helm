...
global:
## Petasus related configuration
  externalDomain: &externalDomain "console.petasus.local"
  HTTPNodePort: &HTTPNodePort 30880
  brand:
    #baseURL: "https://172.16.200.203:30880"
    #baseURL: &baseURL "https://console.petasus.local:30880"
    #baseURL: 'https://{{ .Values.global.externalDomain }}:{{ .Values.global.HTTPNodePort }}'
...
authentik:
  ...
  secret_key: "REPLACE_WITH_LONG_RANDOM_SECRET_1234567890"
  # -- Custom CSS configuration
  customCSS:
    # -- Enable custom CSS deployment from authentik.css file
    enabled: true
  events:
    context_processors:
...
server:
  ...
  image:
    ...
    repository: "ghcr.io/goauthentik/server" # defaults to global.image.repository
    # -- Tag to use to the authentik server
    # @default -- `""` (defaults to global.image.tag)
    tag: "2025.8.4" # defaults to global.image.tag
    ...
    pullPolicy: "IfNotPresent" # defaults to global.image.pullPolicy
  ...
  initContainers:
   - name: wait-for-db
     ...
     command: ["sh","-c","until pg_isready -h ${AUTHENTIK_POSTGRESQL__HOST} -p 5432; do echo waiting for db; sleep 3; done"]
     env:
  ...
  ingress:
    ...
    proxySetHeaders: "ak-proxy-headers"
    # annotations: {}
    annotations:
      nginx.ingress.kubernetes.io/backend-protocol: "HTTP"
      nginx.ingress.kubernetes.io/ssl-redirect: "true"
    #  cert-manager.io/issuer: petasus-auth-selfsigned
      ...
      additionalRules: 
        - matches:
            - path:
                ...
                value: /auth
          backendRefs:
            - name: petasus-access-authentik-server
              port: 80      
          filters:
            - type: ExtensionRef
              extensionRef:  
...
worker:
  ...
  - name: wait-for-redis
    ...
    command: ["sh","-c","until redis-cli -h ${AUTHENTIK_REDIS__HOST} -p 6379 ping | grep PONG; do echo waiting for redis; sleep 3; done"]
    env:
...
ingresscontroller:
  ...
  controller:
    ...
    ingressClassResource:
      name: nginx-petasus-core
      controllerValue: "k8s.io/petasus-nginx"
      default: false
    ingressClass: nginx-petasus-core

    service:
      type: NodePort
      enableHttp: false      # HTTP 비활성
      enableHttps: true      # HTTPS만 사용
      nodePorts:
        https: *HTTPNodePort

# --- nginx Gateway Fabric (Gateway API) Controller configuration        
gatewaycontroller:
  enabled: true
  nginxGateway:
    gatewayClassName: nginx-petasus-core
    snippetsFilters:
      enable: true
    ...
    # 인증서 자동생성 job 사용 여부 (기본 true)
  certGenerator:
    enable: true

  nginx:
    # ingresscontroller.controller.service.type: NodePort 와 동일한 목적
    service:
      type: NodePort
      externalTrafficPolicy: Cluster
      # NodePort는 “Gateway listener port”와 매핑되어야 함
      # (예: listenerPort: 443 에 대한 NodePort 지정)
      nodePorts:
        - port: *HTTPNodePort 
          listenerPort: *HTTPNodePort

  # HTTP 비활성: listener 80을 만들지 않음
  # HTTPS 활성: listener 443만 생성
  gateways:
  - name: nginx-petasus-core
    labels: {}
    annotations: {}
